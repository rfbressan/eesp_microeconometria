...
bhj_ivreg_ss <-
    function(formula,
        ...
        m <-
            match(c("formula", "X", "data", "subset", "weights", "region_cvar"),
                  names(mf),
                  0L)
        mf <- mf[c(1L, m)]
        ...
        Z <- if (stats::is.empty.model(mtZ))
            NULL
        else
            stats::model.matrix(mtZ, mf, contrasts = NULL)
        
        mty2 <-
            stats::delete.response(stats::terms(formula, data = data, rhs = 2))
        attr(mty2, "intercept") <- 0
        y2 <- drop(stats::model.matrix(mty2, mf, contrasts = NULL))
        
        ret <-
            ivreg_ss.fit(y1, y2, mf$"(X)", W, Z, w, method, beta0, alpha, rc,
                         sector_cvar)
        ret$call <- cl
        ret$terms <- mt
        ret
...
ivreg_ss.fit <-
    function(y1,
        ...
        r1 <-
            ...
            stats::lm.wfit(mm, y1, w)
        r2 <-
            if (is.null(w))
                stats::lm.fit(mm, y2)
        else
            stats::lm.wfit(mm, y2, w)
        ...
        if ("all" %in% method)
            method <-
            c("homosk", "ehw", "region_cluster", "akm", "akm0")
        ...
        if ("homosk" %in% method) {
            ...
            den <- sum(wgt * ddX ^ 2)
            se.h <-
                sqrt(rss / NROW(Z)) / (sqrt(den) * abs(r2$coefficients[1]))
        }
        ...
        if ("region_cluster" %in% method)
            if (is.null(region_cvar))
                warning(
                    paste0(
                        "Reporting NA for \"region_cluster\" Std. Error",
                        " because \"region_cvar\" not supplied."
                    )
                )
        else
            se.s <-
            sqrt(drop(crossprod(tapply(
                u, factor(region_cvar), sum
            ))) /
                RX ^ 2)
