...
Bellow we present a table with empirical mean of years of study and the distribution number of children across families, a la Table I in @Manski2000.
...
In table \@ref(tab:lower-bound) we have the estimated lower bound for the causal effect and the 5% bootstrap quantile, as in @Manski2000 Table II.

```{r lower-bound, results='asis'}
kbl(ate_lb_df, digits = 4, booktabs = TRUE,
    caption = "Lower bound on years of study for first child.",
    col.names = c("s", "t", "Estimate", "5% quantile")) %>% 
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 2, "Lower Bound on Delta(s,t)" = 2)) %>% 
  kable_styling(latex_options = c("HOLD_position"))
```
...
**COMPARE TO ATE RESULTS FROM ITEM 1 C)**
