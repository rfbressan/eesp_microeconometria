...
Adding covariates to our model helps making the case for a causal interpretation of $\beta_1$, but it is debatable whether we are including all relevant variables or not. A causal interpretation is due **only** if we control for all factors that affect earnings and are correlated to the worker's choice of entering the union. This is _unlikely_ to be true in the current setup with a limited number of variables, thus, it is still uncertain one can make causal inference with model \@ref(eq:earnings-cov).
...
The sum of weights are 1 for both control and treatment groups. There were no negative values for the weights in control group, although negative values should not come as a surprise according to @Imbens2015b . As for outliers, we better make a boxplot, shown in figure \@ref(fig:weights-box).
...
kbl(num_balance, booktabs = TRUE,
    col.names = c("Variable", "Mean", "Std. Dev.", "Mean", "Std. Dev", "Diff. in Means", "Std.Dev.", "Norm. Diff."),
    caption = "Balance of numerical variables.") %>% 
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 1, "Not Union" = 2, "Union" = 2, "Statistic" = 3))
```

From tables \@ref(tab:cat-balance) and \@ref(tab:num-balance) one can see that numerical variables are well balanced between control and treatment groups, but categorical variables have significant proportion differences.
...
kbl(summary_block_ir, digits = 2, booktabs = TRUE,
  ...
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 1, "Blocks" = 11)) %>% 
...
Indeed many strata of `class_of_worker` type of covariate do not have any observation for a given block, thus the statistics can't be computed, as presented in table \@ref(obs-block-ir) for the IR model only. But this is mainly due to the attribution for the NA block, which we are removing from balance assessment. 
...
The trimming method supressed `r paste(c(sum(!trim_idx_ir), sum(!trim_idx_lasso), sum(!trim_idx_all)), collapse = ", ")` observations from our sample for models IR, Lasso and Full, respectively.
...
We have no such problem in any of our models, but we have to notice the Imbens-Rubin model block #1 has only 3 treated units. This is not a major concern for subclassification without any covariates, but may pose a problem if we stratify further by including covariates in subclassification's regression. 
...
kbl(sub_effect_nc, digits = 2, booktabs = TRUE,
    ...
    col.names = c("Statistic", rep(c("ATE", "ATT"), 3))) %>% 
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 1, "Imbens-Rubin" = 2, "Lasso" = 2, "Full" = 2))
```

Now we will add the three generally most imbalanced covariates after the trimming procedure. Those are `age`, `education` and `private_health_insurance`.

```{r effect, results='asis'}
kbl(sub_effect, digits = 2, booktabs = TRUE,
    caption = "Causal effects contolling by imbalanced covariates.",
    col.names = c("Statistic", rep(c("ATE", "ATT"), 3))) %>% 
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 1, "Imbens-Rubin" = 2, "Lasso" = 2, "Full" = 2))
```

As suspected, inclusion of covariates led Imbens-Rubin model, but also surprisingly the Lasso model, not being able to compute standard errors for the causal effects. The only model from which we are able to make inference, when controlling for some imbalanced covariates, is the Full model. Also, the Full model appears to be more robust to the inclusion of covariates, followed by IR model. The Lasso specification, besides estimating $ATE > ATT$, also had the largest variations in estimates after controlling.
...
kbl(match_nc, digits = 2, booktabs = TRUE,
    caption = "Matching on propensity score. Causal effects without contolling covariates.",
    col.names = c("Model", rep(c("Estimate", "Std.Error", "N.obs", "N.treated", "N.matched"), 2))) %>% 
  kable_classic(full_width = FALSE) %>% 
  add_header_above(c(" " = 1, "ATE" = 5, "ATT" = 5))
```
