...
One can see from above table and comparing to the results obtained from full regression, the p-value for mailing is indeed very close to zero, this treatment being highly significant. Moral and Info treatments, although with different p-values, continue to be not significant. The remarkable change is the p-value for the threat. From a very low p-value, being significant at 1% when asymptotic inference is carried out, the randomization inference attributes an exact p-value of `r format(ppvals_df[2,2], digits = 2)`, and now one can only reject the null hypothesis for this treatment at 10% level of significance.
